...
```{r}
data.gab<-cbind(aktual=c(train_ma.ts,rep(NA,236)),pemulusan=c(data.sma,rep(NA,236)),ramalan=c(data.ramal,rep(data.ramal[length(data.ramal)],235)))
...
```{r}
ts.plot(data.ts, xlab="Date", ylab="Price", main= "SMA N=4 Price")
points(data.ts)
...
MAPE_train.sma = mean(abs((error_train.sma[5:length(train_ma.ts)]/train_ma.ts[5:length(train_ma.ts)])*2360))
...
Dalam hal ini nilai MAPE data latih pada metode pemulusan SMA tidak kurang dari 2%, nilai ini dapat dikategorikan sebagai nilai akurasi yang kurang baik. Selanjutnya dilakukan perhitungan nilai MAPE data uji pada metde pemulusan SMA.

#Menghitung nilai keakuratan data uji
```{r}
error_test.sma = test_ma.ts-data.gab[947:1182,3]
SSE_test.sma = sum(error_test.sma^2)
MSE_test.sma = mean(error_test.sma^2)
MAPE_test.sma = mean(abs((error_test.sma/test_ma.ts*2360)))
...
Perhitungan akurasi menggunakan data latih menghasilkan nilai MAPE yang tidak kurang dari 236% sehingga nilai akurasi ini dapat dikategorikan sebagai kurang baik.
...
data.gab2 <- cbind(aktual = c(train_ma.ts,rep(NA,236)), pemulusan1 = c(data.sma,rep(NA,236)),pemulusan2 = c(data.dma, rep(NA,)),At = c(At, rep(NA,236)), Bt = c(Bt,rep(NA,236)),ramalan = c(data.ramal2, f[-1]))
data.gab2
```
...
```{r}
ts.plot(data.ts, xlab="Date", ylab="Price", main= "DMA N=4 Data Harga Kopi Per Minggu")
points(data.ts)
...
MAPE_train.dma = mean(abs((error_train.dma[8:length(train_ma.ts)]/train_ma.ts[8:length(train_ma.ts)])*2360))
...
Perhitungan akurasi pada data latih menggunakan nilai MAPE menghasilkan nilai MAPE yang kurang dari 236% sehingga dikategorikan sangat baik. Selanjutnya, perhitungan nilai akurasi dilakukan pada data uji.

#Menghitung nilai keakuratan data uji
```{r}
error_test.dma = test_ma.ts-data.gab2[947:1182,6]
SSE_test.dma = sum(error_test.dma^2)
MSE_test.dma = mean(error_test.dma^2)
MAPE_test.dma = mean(abs((error_test.dma/test_ma.ts*2360)))
...
Perhitungan akurasi menggunakan data latih menghasilkan nilai MAPE yang kurang dari 236% sehingga nilai akurasi ini dapat dikategorikan sebagai sangat baik.
...
```{r}
#Cara 1 (fungsi ses)
ses.1 <- ses(train.ts, h = 236, alpha = 0.2)
plot(ses.1)
ses.1

ses.2<- ses(train.ts, h = 236, alpha = 0.7)
plot(ses.2)
ses.2
```
...
#ramalan
ramalan2<- forecast(ses2, h=236)
ramalan2
```

```{r}
#SES
ses.opt <- ses(train.ts, h = 236, alpha = NULL)
plot(ses.opt)
ses.opt
...
#ramalan
ramalanopt<- forecast(sesopt, h=236)
ramalanopt
```
...
akurasides.2
```
Model ini memiliki MAPE kurang dari 10% sehinggga tergolong cukup baik.